...
public class MetricsServer {
    ...
    private static final AtomicLong benchmarkRuns = new AtomicLong(0);
    private static final AtomicLong totalCellsProcessed = new AtomicLong(0);
    private static final AtomicLong totalMemoryBytes = new AtomicLong(0);
    ...
    private static void startMetricsConsumers() {
        ...
        // Consumer for benchmark results
        Properties benchmarkProps = new Properties();
        benchmarkProps.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, BOOTSTRAP_SERVERS);
        benchmarkProps.put(ConsumerConfig.GROUP_ID_CONFIG, "metrics-benchmark-consumer");
        benchmarkProps.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class.getName());
        benchmarkProps.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class.getName());
        benchmarkProps.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "latest");
        benchmarkProps.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "true");
        
        new Thread(() -> {
            try (KafkaConsumer<String, String> consumer = new KafkaConsumer<>(benchmarkProps)) {
                consumer.subscribe(Collections.singletonList("benchmark-results"));
                
                while (true) {
                    var records = consumer.poll(Duration.ofMillis(100));
                    for (var record : records) {
                        try {
                            ResolutionBenchmarkResult result = mapper.readValue(record.value(), ResolutionBenchmarkResult.class);
                            benchmarkRuns.incrementAndGet();
                            totalCellsProcessed.addAndGet(result.cellCount());
                            totalMemoryBytes.addAndGet(result.memoryBytes());
                            lastError = null;
                        } catch (Exception e) {
                            lastError = e.getMessage();
                        }
    ...
    private static String formatJsonMetrics() throws Exception {
        ...
        root.put("averageMatchLatencyMs", Math.round(avgLatency * 100.0) / 100.0);
        root.put("totalDriversFound", totalDriversFound.get());
        root.put("benchmarkRuns", benchmarkRuns.get());
        root.put("totalCellsProcessed", totalCellsProcessed.get());
        root.put("totalMemoryBytes", totalMemoryBytes.get());
...
<head>
    ...
    <title>H3 Resolution Strategy Dashboard</title>
    <style>
...
<body>
    <div id="app" class="container">
        <h1>📊 H3 Resolution Strategy Dashboard</h1>
        ...
        <div class="metrics-grid">
            ...
            <div class="metric-card">
                ...
                <div class="metric-value" id="benchmarkRuns">0</div>
            </div>
            <div class="metric-card">
                <div class="metric-label">Total Cells Processed</div>
                <div class="metric-value" id="totalCellsProcessed">0</div>
            </div>
            <div class="metric-card">
                <div class="metric-label">Total Memory Bytes</div>
                <div class="metric-value" id="totalMemoryBytes">0</div>
            </div>
        </div>
    </div>
    <script>
        ...
        async function fetchMetrics() {
            try {
                ...
                document.getElementById('benchmarkRuns').textContent = formatNumber(data.benchmarkRuns || 0);
                document.getElementById('totalCellsProcessed').textContent = formatNumber(data.totalCellsProcessed || 0);
                document.getElementById('totalMemoryBytes').textContent = formatBytes(data.totalMemoryBytes || 0);
            } catch (error) {
