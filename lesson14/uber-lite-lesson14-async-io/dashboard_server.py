#!/usr/bin/env python3
"""Lesson 14 Dashboard - Async I/O metrics at http://localhost:8082"""
import json
import subprocess
import sys
import threading
...
PORT = 8082
KAFKA_CONTAINER = "lesson14-kafka-1"
...
def _produce_demo_messages():
    ...
    try:
        ...
        for i in range(10):
            msg = '{"driver_id":"demo-%d","latitude":37.77,"longitude":-122.41,"timestamp":%d,"heading":90,"speed_kmh":45.0}\n' % (i, ts_ms + i)
            subprocess.run(
...
def get_metrics():
    ...
    try:
        r = subprocess.run(
            ["docker", "ps", "--filter", "name=lesson14-", "--format", "{{.Names}}"],
            capture_output=True, text=True, timeout=5)
        names = [n for n in (r.stdout or "").strip().splitlines() if n]
        out["containerCount"] = len(names)
        out["brokerCount"] = sum(1 for n in names if "kafka" in n and "kafka-ui" not in n)
...
<head>
  <meta charset="utf-8">
  <title>Lesson 14 – Async I/O Dashboard</title>
  <meta http-equiv="Cache-Control" content="no-cache, no-store, must-revalidate">
  <meta http-equiv="Pragma" content="no-cache">
  <meta http-equiv="Expires" content="0">
  <style>
    html, body { background: #1a1a2e !important; color: #eaeaea; }
    ...
    .card { background: #16213e !important; border: 1px solid #0f3460; padding: 1rem; border-radius: 8px; }
    .card .val { font-size: 1.8rem; font-weight: 700; color: #00d9ff; }
    .error { color: #e94560; }
    .status { font-size: 0.85rem; color: #a0a0a0; margin: 0.25rem 0 0.5rem 0; }
  </style>
</head>
<body>
  <h1>Lesson 14 – Asynchronous I/O Dashboard</h1>
...
def main():
    ...
    try:
        ...
        if "Address already in use" in str(e) or e.errno == 98:
            sys.stderr.write("Port %d in use. Run: pkill -f lesson14.*dashboard_server.py\n" % PORT)
        else:
            sys.stderr.write("Failed to start: %s\n" % e)
        sys.exit(1)
    print("Lesson 14 Dashboard: http://localhost:%d/dashboard" % PORT)
